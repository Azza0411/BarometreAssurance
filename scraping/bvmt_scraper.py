...
EMETTEURS_URL = f"{BASE_URL}/emetteurs"
ESG_REPORTS_URL = f"{BASE_URL}/reporting-esg-societes-cotees"
EDITIONS_STATISTIQUE_URL = f"{BASE_URL}/editions-statistique"
INSTRUMENT_URL_TEMPLATE = f"{BASE_URL}/bourse/instrument/{{isin}}"
...
TITLE_LINK_RE = re.compile(r'views-field-title">\s*<a href="([^"]+)"[^>]*>([^<]+)</a>')
SOCIETE_OPTION_RE = re.compile(r'<option value="(\d+)">([^<]+)</option>')
...
DATE_IN_FILENAME_RE = re.compile(r"\d{2}-\d{2}-(20\d{2})")
...
ISIN_RE = re.compile(r"<td><strong>Code ISIN</strong></td>\s*<td[^>]*>([^<]+)</td>")
MNEMO_RE = re.compile(r"<td><strong>MNEMO</strong></td>\s*<td[^>]*>([^<]+)</td>")
TITRES_EMIS_RE = re.compile(r"<td><strong>Titres .mis</strong></td>\s*<td[^>]*>([^<]+)</td>")
...
BULLETIN_LINK_RE = re.compile(r'href="([^"]*[Bb]ull(\d{8})\.pdf)"')
...
def _fetch_listed_insurance_companies():
    ...
    response = _get_with_retries(
        EMETTEURS_URL, params={"field_secteur_d_activite_target_id": SECTEUR_ASSURANCE_ID}
    )
    return [(name.strip(), path) for path, name in TITLE_LINK_RE.findall(response.text)]
...
def _fetch_esg_societe_ids():
    ...
    response = _get_with_retries(ESG_REPORTS_URL)
    ...
    return {name.strip(): int(societe_id) for societe_id, name in SOCIETE_OPTION_RE.findall(response.text)}


def _fetch_esg_report_links(societe_id):
    """Renvoie les liens PDF des rapports ESG d'une société (URLs absolues)."""
    response = _get_with_retries(ESG_REPORTS_URL, params={"societe": societe_id})
    links = PDF_LINK_RE.findall(response.text)
    return [link if link.startswith("http") else BASE_URL + link for link in links]


def _report_year(pdf_url):
    match = DATE_IN_FILENAME_RE.search(pdf_url) or BARE_YEAR_RE.search(pdf_url)
...
def _matched_insurance_companies():
    ...
    matched = []
    for name, path in _fetch_listed_insurance_companies():
        code = find_code_by_name(name)
        if code:
            matched.append((code, name, path))
        else:
            print(f"  [WARN] Societe BVMT non reconnue dans le registre CMF : {name}")
    return matched
...
def sync_status_cotation():
    ...
    conn = get_connection()
    init_schema(conn)
    source_id = get_or_create_source(conn, "BVMT", BASE_URL)

    companies = _matched_insurance_companies()
    print(f"[STEP] {len(companies)} societe(s) d'assurance cotees reconnues sur BVMT")

    year = datetime.datetime.now().year
    saved = 0
    for code, name, path in companies:
        cmf_id = get_or_create_company(conn, code, name)
        profile_url = path if path.startswith("http") else BASE_URL + path
        document_id = save_document(conn, source_id, cmf_id, f"{code}_profil_bvmt", year, profile_url)
        save_kpi_value(
            conn, document_id, "BVMT - Liste des societes cotees", "Status de cotation", valeur_texte="Cotée"
        )
        saved += 1
...
def sync_esg_documents():
    ...
    companies = _matched_insurance_companies()
    societe_ids = _fetch_esg_societe_ids()

    saved = 0
    for code, name, _path in companies:
        societe_id = societe_ids.get(name)
        if societe_id is None:
            print(f"  [WARN] Pas d'identifiant de filtre ESG trouve pour {name}")
            continue
        cmf_id = get_or_create_company(conn, code, name)
        links = _fetch_esg_report_links(societe_id)
        for link in links:
            year = _report_year(link)
            if year is None:
                print(f"  [WARN] Annee introuvable dans le nom du rapport, ignore : {link}")
                continue
            nom_pdf = f"{code}_ESG_{year}.pdf"
            save_document(conn, source_id, cmf_id, nom_pdf, year, link)
            saved += 1
...
def _bulletin_links_in_range(date_min, date_max):
    ...
    response = _get_with_retries(EDITIONS_STATISTIQUE_URL, params={"date[min]": date_min, "date[max]": date_max})
    matches = {(date_str, path) for path, date_str in BULLETIN_LINK_RE.findall(response.text)}
    return sorted((date_str, path if path.startswith("http") else BASE_URL + path) for date_str, path in matches)
...
def _last_bulletin_of_year(year, today):
    ...
    if year < today.year:
        windows = [(f"{year}-12-15", f"{year}-12-31"), (f"{year}-12-01", f"{year}-12-31"), (f"{year}-01-01", f"{year}-12-31")]
    else:
        windows = [((today - datetime.timedelta(days=30)).isoformat(), today.isoformat())]
    for date_min, date_max in windows:
        links = _bulletin_links_in_range(date_min, date_max)
        if links:
            return links[-1]
    return None, None
...
def sync_market_data():
    ...
    for code, name, path in companies:
        profile_url = path if path.startswith("http") else BASE_URL + path
        try:
            profile_response = _get_with_retries(profile_url)
        except requests.RequestException as exc:
            print(f"  [WARN] Fiche BVMT injoignable pour {code} : {exc}")
            continue
        isin_match = ISIN_RE.search(profile_response.text)
        if not isin_match:
            print(f"  [WARN] Code ISIN introuvable sur la fiche BVMT de {code}")
            continue
        isin = isin_match.group(1).replace(" ", "").strip()

        try:
            instrument_response = _get_with_retries(INSTRUMENT_URL_TEMPLATE.format(isin=isin))
            instrument_html = instrument_response.json().get("html", "")
        except (requests.RequestException, ValueError) as exc:
            print(f"  [WARN] Cotation live BVMT injoignable pour {code} ({isin}) : {exc}")
            continue

        cmf_id = get_or_create_company(conn, code, name)
        document_id = save_document(conn, source_id, cmf_id, f"{code}_profil_bvmt", year_now, profile_url)

        save_kpi_value(conn, document_id, "BVMT - Marche", "Denomination (BVMT)", valeur_texte=name)
        company_kpis_saved += 1

        mnemo_match = MNEMO_RE.search(instrument_html)
        if mnemo_match:
            save_kpi_value(conn, document_id, "BVMT - Marche", "Mnemo (BVMT)", valeur_texte=mnemo_match.group(1).strip())
            company_kpis_saved += 1

        titres_match = TITRES_EMIS_RE.search(instrument_html)
        titres_digits = re.sub(r"\D", "", titres_match.group(1)) if titres_match else ""
        if titres_digits:
            save_kpi_value(
                conn, document_id, "BVMT - Marche", "Nombre d'actions (BVMT)", valeur_nombre=float(titres_digits)
            )
            company_kpis_saved += 1
    ...
    bulletins_saved = 0
    for year in range(BULLETIN_START_YEAR, year_now + 1):
        date_str, bulletin_url = _last_bulletin_of_year(year, today)
        if not bulletin_url:
            print(f"  [WARN] Aucun bulletin trouve pour {year}")
            continue
        save_document(conn, source_id, None, f"bulletin_{year}.pdf", year, bulletin_url)
        bulletins_saved += 1
