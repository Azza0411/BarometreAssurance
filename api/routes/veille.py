...
# ------------------------------------------------------------------ #
_SCRAPE_CACHE: dict = {}  # cache mémoire : clé -> (timestamp, données)
_CACHE_TTL = 3600  # durée de vie du cache (1h)
...
def _cached(key, fn, ttl=_CACHE_TTL):
    now = time.time()  # horodatage courant
    if key in _SCRAPE_CACHE:  # déjà en cache
        ts, data = _SCRAPE_CACHE[key]  # timestamp + données
        if now - ts < ttl:  # encore valide ?
            return data  # renvoie le cache
    data = fn()  # cache expiré -> re-scrape
    _SCRAPE_CACHE[key] = (now, data)  # met à jour le cache
    return data


def _get(url, timeout=6):
    try:
        r = req.get(url, headers=HEADERS, timeout=timeout)  # requête GET
        r.raise_for_status()  # lève si erreur HTTP
        return r  # réponse OK
    except Exception:
...
def _normalize_date(date_str):
    current_year = str(datetime.now().year)  # repli si date illisible
    if not date_str:
        return current_year  # pas de date -> année courante
    date_str = date_str.strip()  # retire les espaces
    if re.match(r'\d{2}/\d{2}/\d{4}', date_str):
        return date_str  # déjà au bon format
    m = re.search(r'(\d{4})-(\d{2})-(\d{2})', date_str)  # format ISO
    if m:
        return f"{m.group(3)}/{m.group(2)}/{m.group(1)}"  # reconverti en JJ/MM/AAAA
    m = re.search(r'(\d{1,2})\s+([a-zéûôàâùè]+)\s+(\d{4})', date_str.lower())  # "12 janvier 2024"
    if m:
        day   = m.group(1).zfill(2)  # jour sur 2 chiffres
        month = MOIS_FR.get(m.group(2)[:3], "01")  # mois FR -> numéro
        return f"{day}/{month}/{m.group(3)}"
    m = re.search(r'\b(20\d{2})\b', date_str)  # au moins une année trouvée
    if m:
        return m.group(1)  # année en dernier recours
    return current_year  # rien trouvé


def _categorize(titre):
    # Classement heuristique par mots-clés présents dans le titre (aucun NLP)
    t = titre.lower()  # insensible à la casse
...
def _article_image(url):
    ...
    if not r:
        return None, ""  # page inaccessible
    soup = BeautifulSoup(r.text, "html.parser")  # parse la page
    desc_tag = soup.find("meta", property="og:description") or soup.find("meta", attrs={"name": "description"})  # meta description
    desc = (desc_tag.get("content", "") if desc_tag else "").strip()  # texte du résumé
    og = soup.find("meta", property="og:image")  # image Open Graph
    if og and og.get("content"):
        return og["content"], desc  # priorité à og:image
    for sel in ["article img", ".article-content img", "figure img", ".entry-content img", ".post img"]:
        img = soup.select_one(sel)  # sélecteurs CSS de repli
        if img:
            src = img.get("src") or img.get("data-src", "")  # src ou lazy-load
            if src and not src.endswith(".gif"):  # exclut les gifs
                if not src.startswith("http"):
                    domain = re.match(r'https?://[^/]+', url)  # domaine de la page
                    src = (domain.group() if domain else "") + src  # URL absolue
                return src, desc
    return None, desc  # pas d'image, résumé quand même
...
def _scrape_ilboursa():
    ...
    ILBOURSA_TICKERS = [
        ("cotation_STAR",  "STAR Assurances"),   # page IlBoursa par ticker
    ...
    seen_urls    = set()  # anti-doublon inter-tickers
    article_links = []  # liste brute avant enrichissement

    for ticker, compagnie_default in ILBOURSA_TICKERS:
        r = _get(f"https://www.ilboursa.com/marches/{ticker}")  # page cotation
        if not r:
            continue  # page injoignable
        soup = BeautifulSoup(r.text, "html.parser")  # parse la page
        container = soup.find("div", class_="lh25")  # bloc des actualités liées
        if not container:
            continue  # structure inattendue

        pending_date = ""  # date "en attente" : avant le lien, pas dedans
        for child in container.children:  # parcourt span date, puis lien
            from bs4 import NavigableString, Tag
            if isinstance(child, Tag) and child.name == "span" and "sp1" in (child.get("class") or []):
                pending_date = child.get_text(strip=True)  # date du prochain lien
            elif isinstance(child, Tag) and child.name == "a":
                href = child.get("href", "")
                if not re.search(r"/marches/.+_\d+$", href):
                    continue  # pas un lien d'actualité
                if not href.startswith("http"):
                    href = "https://www.ilboursa.com" + href  # URL absolue
                if href in seen_urls:
                    continue  # déjà vu
                titre = child.get_text(strip=True)
                if len(titre) < 15:
                    continue  # titre trop court
                seen_urls.add(href)
                titre_l    = titre.lower()
                compagnie  = compagnie_default  # défaut = ticker consulté
                for co in INSURANCE_COMPANIES:
                    if any(k in titre_l for k in co["keys"]):
                        compagnie = co["name"]  # autre compagnie nommée
                        break
                article_links.append((titre, href, pending_date, compagnie))
                pending_date = ""  # consommée

    def fetch_one(item):
        # Enrichit un article brut : visite sa page pour récupérer image + résumé
        titre, href, date_str, compagnie = item
        img, resume = _article_image(href)  # 2e requête HTTP
        if re.match(r"\d{1,2}/\d{1,2}/\d{2}$", date_str):
            parts = date_str.split("/")
            parts[2] = "20" + parts[2]  # année sur 4 chiffres
            date_str = "/".join(parts)
        return {
            "src":       "ILBOURSA",
            "titre":     titre,
            "url":       href,
            "date":      _normalize_date(date_str),  # format JJ/MM/AAAA
            "categorie": _categorize(titre),  # déduite par mots-clés
            "compagnie": compagnie,
            "resume":    resume,
            "image":     img,
            "pdf_url":   None,  # pas de PDF sur IlBoursa
        }

    with ThreadPoolExecutor(max_workers=8) as executor:  # 8 enrichissements en parallèle
        futures = {executor.submit(fetch_one, item): item for item in article_links[:50]}  # limite à 50
        for future in as_completed(futures):  # résultats au fur et à mesure
            try:
                articles.append(future.result())
            except Exception:
                pass  # échec individuel isolé
    return articles
...
def _scrape_atlas():
    ...
    cutoff_year  = datetime.now().year - 5  # ignore les plus vieux de 5 ans
    article_links = []  # liste brute avant enrichissement

    for pg in range(4):  # 4 premières pages
        url = "https://www.atlas-mag.net/fr/news/tunisia" + (f"?page={pg}" if pg > 0 else "")  # pagination
        r   = _get(url)
        if not r:
            continue  # page injoignable
        soup = BeautifulSoup(r.text, "html.parser")
        main_cards = [
            c for c in soup.find_all("div", class_="card")  # cartes d'articles
            if "sidebar" not in " ".join(c.parent.get("class", []))  # exclut la sidebar
        ]
        for card in main_cards:
            title_tag = card.find("h5") or card.find(["h2", "h3", "h4"])  # titre
            titre = title_tag.get_text(strip=True) if title_tag else ""

            href = None
            for a in card.find_all("a", href=True):
                h = a["href"]
                if "/fr/articles/" in h:  # lien vers un vrai article
                    if not h.startswith("http"):
                        h = "https://www.atlas-mag.net" + h  # URL absolue
                    href = h
                    break

            if not href or not titre or len(titre) < 8 or href in seen_urls:
                continue  # carte incomplète ou déjà vue

            time_tag = card.find("time")  # date de publication
            date_str = ""
            if time_tag:
                date_str = time_tag.get("datetime", "") or time_tag.get_text(strip=True)  # attribut prioritaire

            norm_date = _normalize_date(date_str)  # format JJ/MM/AAAA
            yr = re.search(r'\b(20\d{2})\b', norm_date)
            if yr and int(yr.group()) < cutoff_year:
                continue  # article trop ancien

            titre_l   = titre.lower()
            compagnie = "—"  # pas de défaut ici
            for co in INSURANCE_COMPANIES:
                if any(k in titre_l for k in co["keys"]):
                    compagnie = co["name"]  # compagnie nommée dans le titre
                    break
    ...
    def fetch_atlas_one(item):
        ...
        return {
            ...
            "url":       href,
            "date":      norm_date,
            "categorie": cat if cat != "Actualité" else "Publication",  # défaut Atlas
            "compagnie": compagnie,
            "resume":    resume,
            "image":     img,
            "pdf_url":   None,
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:  # enrichissement en parallèle
...
def _detect_type(text):
    ...
    return "Texte"  # non reconnu


def _extract_date_from_title(titre):
    # Les textes réglementaires portent leur date dans le titre (ex: "...du 12 mars 2020")
    t = titre.lower()
    m = re.search(r'\bdu\s+(\d{1,2})(?:er)?\s+([a-zéûôàâùè]+)\s+(\d{4})', t)  # "du JJ(er) mois AAAA"
    if m:
        day   = m.group(1).zfill(2)  # jour sur 2 chiffres
        month = MOIS_FR.get(m.group(2)[:3], None) or MOIS_FR.get(m.group(2)[:4], "01")  # mois -> numéro
        return f"{day}/{month}/{m.group(3)}", int(m.group(3))  # date + année
    m = re.search(r'\b(\d{4})\b', titre)  # sinon année isolée
    if m:
...
def _scrape_cga_page(page_id):
    ...
    soup     = BeautifulSoup(r.content, "html.parser")  # bytes, BS4 gère l'encodage
    docs     = []
    seen_pdfs = set()  # anti-doublon par nom de fichier

    for a_tag in soup.find_all("a", href=True):
        href = a_tag["href"]
        if "fileadmin" not in href or ".pdf" not in href.lower():
            continue  # que les PDF officiels
        if not href.startswith("http"):
            href = "https://www.cga.gov.tn/" + href.lstrip("/")  # URL absolue
        fname = href.split("/")[-1].lower()  # nom de fichier, clé de dédoublonnage
        if fname in seen_pdfs:
            continue  # déjà traité
        seen_pdfs.add(fname)

        container = a_tag.parent  # remonte vers un bloc avec le titre complet
        for _ in range(5):  # au plus 5 niveaux
            if container is None:
                break
            tag = getattr(container, "name", None)
            if tag in ("li", "p", "td", "article"):
                break  # conteneur "logique" trouvé
            container = getattr(container, "parent", None)
        if container is None:
            container = a_tag.parent  # repli : parent direct

        titre = re.sub(r'\s+', ' ', container.get_text(separator=" ", strip=True)).strip()  # texte normalisé
        if not titre or len(titre) < 5:
            titre = a_tag.get_text(strip=True) or fname.replace("_", " ").replace(".pdf", "")  # repli
        if len(titre) > 200:
            titre = titre[:200].rsplit(" ", 1)[0] + "…"  # tronque proprement

        type_      = _detect_type(titre)  # type déduit du titre
        date, annee = _extract_date_from_title(titre)  # date/année du titre
        docs.append({
            "id":      hashlib.md5(fname.encode()).hexdigest()[:12],  # id basé sur le nom de fichier
            "src":     "CGA",
            "type":    type_,
            "titre":   titre,
            "url":     href,
...
def _scrape_ftusa_textes():
    ...
    for tag in soup.find_all(["nav", "header", "footer"]):
        tag.decompose()  # retire menu/en-tête/pied de page

    for container in soup.find_all(["li", "p"]):  # une entrée = un <li> ou <p>
        text = re.sub(r'\s+', ' ', container.get_text(separator=" ", strip=True)).strip()  # texte normalisé
        if not text or len(text) < 8 or len(text) > 300:
            continue  # trop court ou trop long
        text_l = text.lower()
        if not any(k in text_l for k in ["loi ", "décret", "decret", "arrêté", "arrete",
                                          "ordonnance", "code des", "décision", "règlement"]):
            continue  # ne ressemble pas à un texte réglementaire
        type_ = _detect_type(text)
        if type_ == "Texte":
            continue  # type non reconnu, écarté par prudence

        a_tag = container.find("a", href=True)  # lien éventuel
        if a_tag:
            href = a_tag["href"]
            if not href.startswith("http"):
                href = "https://www.ftusanet.org" + (href if href.startswith("/") else "/" + href)  # URL absolue
        else:
            href = url  # pas de lien -> renvoie vers la page

        if href in seen:
            continue  # déjà traité
        is_pdf      = href.lower().endswith(".pdf")  # lien direct vers un PDF ?
        date, annee = _extract_date_from_title(text)  # date/année de l'entrée
        seen.add(href)
        docs.append({
            "id":      hashlib.md5((text[:80] + href).encode()).hexdigest()[:12],  # id texte+url
            "src":     "FTUSA",
            "type":    type_,
            "titre":   text if len(text) <= 200 else text[:200].rsplit(" ", 1)[0] + "…",
            "url":     href,
            "pdf_url": href if is_pdf else None,  # None si pas un PDF direct
            "date":    date,
            "annee":   annee,
        })
    return docs
...
def _scrape_ftusa_code():
    ...
    r   = _get(url)
    if not r:
        # page injoignable -> entrée de repli avec infos connues à l'avance
        return [{
    ...
    soup    = BeautifulSoup(r.text, "html.parser")
    heading = soup.find(["h1", "h2", "h3"])  # premier gros titre trouvé
    titre   = heading.get_text(strip=True) if heading else "Code des assurances"
    if not titre or len(titre) < 5:
        titre = "Code des assurances"  # repli

    pdf_url = None
    for a in soup.find_all("a", href=True):
        if ".pdf" in a["href"].lower():
            pdf_url = a["href"]  # premier lien PDF trouvé
            if not pdf_url.startswith("http"):
                pdf_url = "https://www.ftusanet.org" + pdf_url  # URL absolue
            break

    return [{
        "id": "ftusa_code_ass", "src": "FTUSA", "type": "Code",
        "titre": titre if len(titre) > 8 else "Code des assurances (Loi n°92-24 du 9 mars 1992)",
        "url": url, "pdf_url": pdf_url, "date": "09/03/1992", "annee": 1992,  # date de la loi, codée en dur
    }]
...
def _build_veille():
    ...
    all_docs = []
    with ThreadPoolExecutor(max_workers=4) as ex:  # une source par thread
        futures = [ex.submit(fn) for fn in scrapers]
        for fut in as_completed(futures, timeout=20):  # 20s max au total
            try:
                all_docs.extend(fut.result())
            except Exception:
                pass  # une source en échec n'en bloque pas d'autres

    seen_keys = set()  # dédoublonnage par id/url ET par début de titre
    deduped   = []
    for d in all_docs:
        title_key = re.sub(r'\s+', ' ', d["titre"].lower().strip())[:80]  # empreinte du titre
        key       = d.get("id") or d["url"]
        if key not in seen_keys and title_key not in seen_keys:
            seen_keys.add(key)
            seen_keys.add(title_key)
            deduped.append(d)  # même texte gardé une seule fois
    ...
    def sort_key(d):
        ...
        parts = date.split("/")
        if len(parts) == 3:
            return f"{parts[2]}{parts[1]}{parts[0]}"  # JJ/MM/AAAA -> AAAAMMJJ
        annee = d.get("annee")
        return str(annee) if annee else "0000"  # sans date, tri en dernier
...
def sync_new_items():
    ...
    *_vues (database.repository) pour détecter ce qui est réellement
    ...
    try:
        ...
        conn.close()  # ferme même en cas d'exception
